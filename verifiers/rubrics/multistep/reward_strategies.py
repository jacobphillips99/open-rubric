    ...
    def calculate_reward(self, result: Union[Dict[str, Any], 'EvaluationResult'], 
        ...
        for level_idx, level_scores in state.items():
            if isinstance(level_scores, dict):
                level_sum = sum(level_scores.values())
                weight = self.base_weight + (level_idx * self.level_multiplier)
...
class LevelBasedRewardStrategy(RewardStrategy):
    ...
    def calculate_reward(self, result: Union[Dict[str, Any], 'EvaluationResult'], 
        ...
        if isinstance(result, EvaluationResult):
            state = result.state
            completion_ratio = result.completion_ratio if hasattr(result, 'completion_ratio') else 0.0
        else:
            state = result
        ...
        max_level = max(state.keys()) if state else 0
...
class CompletionRatioRewardStrategy(RewardStrategy):
    ...
    def calculate_reward(self, result: Union[Dict[str, Any], 'EvaluationResult'], 
        ...
        if isinstance(result, EvaluationResult):
            ...
            completed = sum(len(level_scores) for level_scores in state.values() 
                          if isinstance(level_scores, dict))
        ...
        quality_reward = (total_score / count if count > 0 else 0.0) * self.quality_weight
...
class ProgressiveRewardStrategy(RewardStrategy):
    ...
    def calculate_reward(self, result: Union[Dict[str, Any], 'EvaluationResult'], 
        ...
        for level_idx, level_scores in state.items():
            if isinstance(level_scores, dict):
                level_multiplier = self.base_reward * (self.growth_factor ** level_idx)
                level_sum = sum(level_scores.values())
