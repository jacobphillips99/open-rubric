...
from verifiers.rewards.judge_reward import JUDGE_PROMPT, BinaryJudgeRewarder, JudgeResponse
from verifiers.envs.multistep_env import MultiStepMultiTurnEnv

from multistep_extras.utils.print_utils import (
    Colors,
    print_header, print_section, print_success, print_error, print_state,
    print_assistant, print_environment, print_info, print_process,
    print_rubric, print_score, print_reward, print_debug,
)
...
def _print_state_transitions(state_before: dict, state_after: dict) -> None:
    ...
    new_revealed = state_after.get('revealed_info', set()) - state_before.get('revealed_info', set())
    
    if level_change > 0:
        print_score(f"LEVEL PROGRESSION: {state_before['level_idx']} → {state_after['level_idx']} (+{level_change})")
    ...
    if last_results:
        ...
        for req_name, result_data in last_results.items():
            if isinstance(result_data, dict):
                ...
                print(f"   {Colors.DEBUG}• {req_name}: {answer_color}Answer={answer}{Colors.END}")
                if reasoning:
                    # Truncate reasoning for display
                    reasoning_preview = _truncate_content(reasoning, 100)
                    print(f"     {Colors.DEBUG}Reasoning: {reasoning_preview}{Colors.END}")
            else:
                # Handle old format or direct values
                answer_color = Colors.SUCCESS if result_data == 1.0 else Colors.ERROR
                print(f"   {Colors.DEBUG}• {req_name}: {answer_color}Answer={result_data}{Colors.END}")
    ...
    if state['active_reqs'] and rubric is not None:
        ...
        for req_name in state['active_reqs']:
            if req_name in rubric.name_to_req:
                ...
                print(f"   {Colors.DEBUG}• {req_name}: terminal={terminal}, deps={deps}{Colors.END}")

def print_conversation_analysis(completion: List[Dict[str, Any]], final_state: Dict[str, Any], rubric=None, max_content_length: int = 500) -> None:
    ...
    for i, msg in enumerate(completion):
        role_color = Colors.ASSISTANT if msg['role'] == 'assistant' else Colors.ENVIRONMENT
        content_preview = _truncate_content(msg['content'], max_content_length)
        print(f"{role_color}{i+1}. {msg['role'].upper()}: {content_preview}{Colors.END}")

def print_workflow_progression_from_state(final_state: dict, rubric=None, max_content_length: int = 300) -> None:
    ...
    for step in progression:
        ...
        if step_type == "initial_prompt":
            ...
            if "content" in step and step["content"]:
                ...
                print_environment("No explicit environment response - letting model continue naturally")
    ...
    for level_str, level_results in evaluation_data.items():
        ...
        if isinstance(level_results, dict):
            for req_name, result_data in level_results.items():
                ...
                answer_color = Colors.SUCCESS if result_data['answer'] == 1.0 else Colors.ERROR
                print(f"    {answer_color}Score: {result_data['answer']}{Colors.END}")
                
                if result_data['reasoning']:
                    print(f"    {Colors.DEBUG}Reasoning: {result_data['reasoning']}{Colors.END}")
            
        else:
            print_debug(f"    Result: {level_results}")


def test_multistep_rollout(client: OpenAI, model: str,requirements: list[Requirement], scenario: Scenario):
    ...
    try:
        ...
        if revealed_info:
            print_info(f"Information revealed during conversation: {len(revealed_info)} items")
...
def test_state_tracking(client: OpenAI, requirements: list[Requirement], scenario: Scenario):
    ...
    print_info(f"  Level progression: {final_state['level_idx'] - initial_state['level_idx']}")
    print_info(f"  Progression steps captured: {len(final_state.get('progression', []))}")
