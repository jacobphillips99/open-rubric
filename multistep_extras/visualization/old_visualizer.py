...
class RequirementsVisualizer(BaseRequirementsInspector):
    ...
    def create_dependency_graph(
        ...
        self._add_edges_to_figure(
            fig, edges, positions, show_answer_labels, highlight_path
        )
        ...
        self._add_nodes_to_figure(
            ...
            positions,
            highlight_path,
            show_terminal_states,
            show_requirement_types,
        )
                ...
                for answer, deps in req.dependencies.items():
                    for dep in deps:
                        edge = {
                            ...
                            "answer": answer,
                            "label": str(answer),
                            "source_type": req.__class__.__name__.replace(
                                "Requirement", ""
                            ).lower(),
                            "target_type": next(
                                (
                                    r.__class__.__name__.replace(
                                        "Requirement", ""
                                    ).lower()
    ...
    def _add_edges_to_figure(
        ...
        for edge in edges:
            ...
            is_highlighted = (
                highlight_path
                and source_req in highlight_path
                and float(highlight_path[source_req]) == float(answer)
            )
    ...
    def create_metrics_dashboard(self) -> go.Figure:
        ...
        fig.add_trace(
            go.Histogram(
                x=dep_counts, nbinsx=10, name="Dependencies", marker_color="#3498db"
            ),
            row=2,
            col=1,
        )
        ...
        terminal_analysis = [
            ...
            (
                "Non-Terminal by Type",
                len([r for r in self.requirements if not r.terminal()]),
            ),
        ...
        fig.add_trace(
            go.Bar(
                ...
                y=[
                    s[1] if isinstance(s[1], (int, float)) else 0
                    for s in terminal_analysis
                ],
...
def create_dependency_graph(requirements: List[Requirement], **kwargs) -> go.Figure:
    """
...
def create_rubric_dependency_graph(rubric: MultiStepRubric, **kwargs) -> go.Figure:
    """
...
def create_path_visualization(
    requirements: List[Requirement], answers: Dict[str, float], **kwargs
) -> go.Figure:
    """
