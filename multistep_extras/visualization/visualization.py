...
class WorkflowVisualizer:
    ...
    def analyze_workflow_metrics(self) -> Dict[str, Any]:
        ...
        total_branches = sum(
            len(req.dependencies) for req in branching_nodes
        )
        avg_branching_factor = total_branches / len(branching_nodes) if branching_nodes else 0
    ...
    def trace_evaluation_path(self, answers: Dict[str, float]) -> List[Tuple[int, List[str]]]:
        """
        ...
        while current_requirements and current_level < len(self.levels):
            ...
            for req_name in current_requirements:
                if req_name in answers:
                    ...
                    if not req.terminal() and answer in req.dependencies:
                ...
                for req_name in requirements:
                    if req_name in answers:
                        ...
                        if not req.terminal() and answer in req.dependencies:
                            ...
                            if active_deps:
                                print(f"  {req_name} ({answer}) → {', '.join(active_deps)}")
        print()
...
def visualize_workflow(requirements: List[Requirement], 
...
def compare_workflows(workflow1: List[Requirement], 
                     workflow2: List[Requirement],
                     names: Tuple[str, str] = ("Workflow 1", "Workflow 2")) -> None:
    """
