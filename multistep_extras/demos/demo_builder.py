...
def demo_fluent_builder():
    ...
    builder.node(
        "greet_customer", "Does the response include a proper greeting?"
    ).if_yes("identify_issue").if_no("request_clarification")

    builder.node(
        "identify_issue", "Does the response identify the customer's main issue?"
    ).if_yes("check_account_status", "gather_details").if_no("ask_clarifying_questions")

    builder.node(
        "check_account_status", "Does the response check the customer's account?"
    ).if_yes("provide_solution").if_no("escalate_to_specialist")

    builder.node("gather_details", "Does the response gather relevant details?").if_yes(
        "provide_solution"
    ).if_no("ask_clarifying_questions")

    # Terminal nodes
    builder.node(
        "provide_solution", "Does the response provide a clear solution?"
    ).terminal()
    builder.node(
        "escalate_to_specialist", "Does the response escalate appropriately?"
    ).terminal()
    builder.node(
        "ask_clarifying_questions", "Does the response ask for clarification?"
    ).terminal()
    builder.node(
        "request_clarification", "Does the response request clarification?"
    ).terminal()
...
def demo_linear_template():
    ...
    steps = [
        ...
        (
            "document_solution",
            "Does the response document the solution for future reference?",
        ),
    ]
...
def demo_quick_helpers():
    ...
    workflow = quick_workflow(
        "analyze_situation", "plan_response", "execute_plan", "evaluate_results"
    )
...
def demo_scenario_builder():
    ...
    scenario = (
        ...
        .prompt(
            "A fire alarm is going off in the building. People are starting to panic."
        )
        .completion(
            "I'll immediately assess if it's a real emergency, guide people to nearest exits calmly, and ensure everyone follows evacuation procedures."
        )
